...
export default function CardProdInCart({ item, setProductToDelete, setShowConfirm }) {
  ...
  return (
    <div className="bg-[#2F4A2F] backdrop-blur-sm p-4 md:p-6 shadow-sm flex gap-4 md:gap-6 items-center border-b border-[#E8D6B3]/10">
      ...
      <div className="w-24 h-24 md:w-32 md:h-32 overflow-hidden shrink-0 flex items-center justify-center">
        <img src={item.imagen} alt={item.nombre} className="w-50 h-50 object-contain" />
      </div>

      <div className="flex-grow">
        <h3 className="font-belleza text-xl md:text-2xl text-[#E8D6B3] leading-tight tracking-wide">{item.nombre}</h3>
        ...
        <div className="flex items-center gap-3 mt-4 md:hidden">
          <div className="flex items-center border border-[#2F4A2F]/20 bg-[#F2E4C9] shadow-inner overflow-hidden">
            <button onClick={() => updateQuantity(item.id, item.colorSeleccionado, -1)} className="p-2 hover:bg-[#8B5E3C]/10 transition-colors"><Minus size={16}/></button>
            <span className="px-3 font-bold text-[#2F4A2F] min-w-[30px] text-center">{item.cantidad}</span>
            <button 
              onClick={() => updateQuantity(item.id, item.colorSeleccionado, 1)}
              disabled={item.cantidad >= item.stock}
              className={`p-2 hover:bg-[#8B5E3C]/10 transition-colors ${item.cantidad >= item.stock ? 'opacity-20' : ''}`}
            >
              <Plus size={16}/>
            </button>
          </div>
