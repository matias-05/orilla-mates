...
import CardProdInCart from './CardProdInCart';
...
export default function CarritoLleno() {
    ...
    return (
        <section className="bg-[#F2E4C9] min-h-[calc(100dvh-80px)] font-quicksand py-8 md:py-16 px-4 md:px-12 relative">
          <AlertaBorrar showConfirm={showConfirm} setShowConfirm={setShowConfirm} productToDelete={productToDelete} setProductToDelete={setProductToDelete} />
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
              <div className="lg:col-span-2 space-y-4">
                {cart.map((item) => (
                  <CardProdInCart key={`${item.id}-${item.colorSeleccionado}`} item={item} setProductToDelete={setProductToDelete} setShowConfirm={setShowConfirm}/>
                ))}
              </div>
              <CardResumen />
            </div>
