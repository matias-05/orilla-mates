    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 w-full">
      {categorias.map((cat) => (
        <Link
          key={cat.id}
          to={cat.path}
          className="group relative aspect-[3/4] overflow-hidden  cursor-pointer shadow-2xl block"
        >
          <img
            src={cat.imagen}
            alt={cat.nombre}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-4 md:p-8">
            ...
            <span className="text-[#E8D6B3] text-[10px] md:text-xs text-center mt-2 tracking-[0.3em] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              Ver más
            </span>
          </div>
        </Link>
